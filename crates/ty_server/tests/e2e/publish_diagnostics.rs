...
fn deferred_opens_complete_with_one_worker() -> Result<()> {
    ...
    let report = server.document_diagnostic_request(&paths[0], None);
    let lsp_types::DocumentDiagnosticReport::RelatedFullDocumentDiagnosticReport(report) = report
    else {
        anyhow::bail!("expected a complete diagnostic report");
    };
...
fn opening_and_closing_manifest_preserves_project_diagnostics() -> Result<()> {
    ...
    assert_eq!(
        ...
        Some(lsp_types::Code::String("unknown-rule".to_string()))
    );
    ...
    server.write_file(manifest, "[tool.ty.rules]\n")?;
    ...
    assert_eq!(cleared.uri, initial.uri);
