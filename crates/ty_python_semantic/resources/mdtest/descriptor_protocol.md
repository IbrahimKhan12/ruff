...
In Python 3.9–3.12, `classmethod` can wrap another descriptor. Attribute access passes the class to
the wrapped descriptor, so a property getter receives the class on both class and instance access.
...
reveal_type(C.units)  # revealed: tuple[str, ...]
reveal_type(C().units)  # revealed: tuple[str, ...]

class D(C): ...

reveal_type(D.units)  # revealed: tuple[str, ...]
reveal_type(D().units)  # revealed: tuple[str, ...]
```

### Class methods wrapping custom descriptors

Descriptor forwarding also applies to user-defined descriptors, without requiring the wrapped object
to be callable. Its `__get__` method receives the class as both arguments.
...
The forwarded arguments still need to match the descriptor's signature. A descriptor that expects an
integer instead of the class reports an error on access.

```py
class InvalidDescriptor:
    def __init__(self, function: Callable[..., object]) -> None: ...
    def __get__(self, instance: int, owner: type) -> str:
        return str(instance)
...
# error: [invalid-attribute-access] "Expected `int`, found `<class 'D'>`"
D.name
```

### Class methods wrapping properties in Python 3.13

Python 3.13 removed descriptor forwarding from `classmethod`. A property no longer provides a valid
...
```py
class C:
    # error: [invalid-argument-type] "found `property`"
    @classmethod
    @property
    def units(cls) -> tuple[str, ...]:
        return ("year", "month")
