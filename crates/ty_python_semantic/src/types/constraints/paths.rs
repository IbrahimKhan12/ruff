...
impl PathAssignments {
    ...
    fn discover_single_constraint<'db>(
        ...
        let constraints = constraint.into_new();
    ...
    fn discover_constraint_pair<'db>(
        ...
        if let (InterimConstraint::Old(left), InterimConstraint::Old(right)) = (left, right) {
            let implies = |left: OldConstraint<'db>, right: OldConstraint<'db>| {
                ...
                let left_upper = left.bounds.upper_bound().ty().top_materialization(db, env);
                let right_lower = right
                    .bounds
                    .lower_bound()
                    .ty()
                    .bottom_materialization(db, env);
                let right_upper = right.bounds.upper_bound().ty().top_materialization(db, env);
