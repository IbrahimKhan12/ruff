...
fn database(rule: Option<&str>, modules: &[&str]) -> anyhow::Result<TestDb> {
    let options = rule.map(|rule| format!("[rules]\nunused-dependency = '{rule}'"));
    database_with_options(options.as_deref(), modules, ValueSource::Cli)
}
...
fn declarations(db: &TestDb, diagnostics: &[Diagnostic]) -> anyhow::Result<Vec<(String, String)>> {
    diagnostics
        .iter()
        .map(|diagnostic| {
...
#[test]
fn project_declaration_range_and_severity() -> anyhow::Result<()> {
    let mut db = database(Some("error"), &["unused_lib"])?;
    db.write_file(
        "/project/pyproject.toml",
        "[project]\ndependencies = ['Unused.Lib>=1']\n",
    )?;
...
fn disabled_by_default() -> anyhow::Result<()> {
    let mut db = database(None, &["unused_lib"])?;
    db.write_file(
        "/project/pyproject.toml",
...
#[test]
fn dependencies_without_known_modules_are_not_reported() -> anyhow::Result<()> {
    let mut db = database(Some("warn"), &[])?;
    db.write_file(
        "/project/pyproject.toml",
        "[project]\ndependencies = ['command-tool']\n",
    )?;
    db.write_file("/project/main.py", "")?;
...
fn unopened_file_import_counts_for_project() -> anyhow::Result<()> {
    let mut db = database(Some("warn"), &["used_lib"])?;
    db.write_file(
        "/project/pyproject.toml",
...
fn local_module_shadowing_does_not_use_installed_distribution() -> anyhow::Result<()> {
    let mut db = database(Some("warn"), &["shared_lib"])?;
    db.write_file(
        "/project/pyproject.toml",
...
fn runtime_and_optional_declarations_exclude_groups_and_markers() -> anyhow::Result<()> {
    ...
    db.write_file(
        "/project/pyproject.toml",
        "[project]\ndependencies = ['unused-lib', \"conditional-lib; python_version < '3.0'\"]\n\n[project.optional-dependencies]\nfeature = ['optional-lib']\n\n[dependency-groups]\ndev = ['dev-tool']\n",
    )?;
    db.write_file("/project/main.py", "")?;
    let mut metadata = metadata(&[(
        "/project",
    ...
    assert_eq!(
        declarations(&db, &project_diagnostics(&db, &metadata))?,
        [
            ("/project/pyproject.toml".into(), "'unused-lib'".into()),
...
#[test]
fn partial_check_does_not_report_unused_dependencies() -> anyhow::Result<()> {
    let mut db = database(Some("warn"), &["unused_lib"])?;
    db.write_file(
        "/project/pyproject.toml",
        "[project]\ndependencies = ['unused-lib']\n",
    )?;
    ...
    let metadata = metadata(&[("/project", DependencyProjectKind::Project, &["unused-lib"])])?;
    let project = db.project();
    for path in ["/project/src", "/project/src/main.py"] {
        project.set_included_paths(&mut db, vec![SystemPathBuf::from(path)]);
        assert!(project_diagnostics(&db, &metadata).is_empty());
    }
    project.set_included_paths(&mut db, vec![SystemPathBuf::from("/project")]);
    assert_eq!(project_diagnostics(&db, &metadata).len(), 1);
    Ok(())
}
...
fn custom_source_filters_do_not_prove_unused_dependencies() -> anyhow::Result<()> {
    for (filter, source) in [
        ...
        ("include = ['/project/src']", ValueSource::Cli),
        ("exclude = ['/project/worker.py']", ValueSource::Cli),
    ] {
        let options = format!("[rules]\nunused-dependency = 'warn'\n[src]\n{filter}\n");
        let mut db = database_with_options(Some(&options), &["used_lib"], source)?;
        db.write_file(
            "/project/pyproject.toml",
            "[project]\ndependencies = ['used-lib']\n",
        )?;
        ...
        assert_eq!(
            script_diagnostics(
                &db,
                script(&db, "/project/src/script.py")?,
                &script_metadata,
            )
            .len(),
            1
...
#[test]
fn complete_member_check_and_sibling_isolation() -> anyhow::Result<()> {
    let mut db = database(Some("warn"), &["shared_lib"])?;
    for member in ["a", "b"] {
        db.write_file(
    ...
    db.write_file("/project/a/main.py", "")?;
...
#[test]
fn script_import_does_not_use_parent_project_dependency() -> anyhow::Result<()> {
    let mut db = database(Some("warn"), &["shared_lib"])?;
    db.write_file(
        "/project/pyproject.toml",
        "[project]\ndependencies = ['shared-lib']\n",
...
#[test]
fn script_does_not_count_unreachable_file_imports() -> anyhow::Result<()> {
    let mut db = database(Some("warn"), &["unused_lib"])?;
    db.write_file(
        "/project/script.py",
        "# /// script\n# dependencies = ['unused-lib']\n# ///\n",
    )?;
    db.write_file("/project/unrelated.py", "import unused_lib\n")?;
    let metadata = metadata(&[(
        "/project/script.py",
        DependencyProjectKind::Script,
        &["unused-lib"],
    ...
    assert_eq!(
        script_diagnostics(&db, script(&db, "/project/script.py")?, &metadata).len(),
        1
    );
    Ok(())
}
...
#[test]
fn script_checks_implementation_hidden_by_local_stub() -> anyhow::Result<()> {
    let mut db = database(Some("warn"), &["used_lib"])?;
    db.write_file(
        "/project/script.py",
        "# /// script\n# dependencies = ['used-lib']\n# ///\nimport helper\n",
    ...
    db.write_file("/project/helper.py", "import used_lib\n")?;
    let metadata = metadata(&[(
        "/project/script.py",
        DependencyProjectKind::Script,
        &["used-lib"],
...
#[test]
fn script_follows_helpers_in_extra_search_paths() -> anyhow::Result<()> {
    let mut db = database(Some("warn"), &["used_lib"])?;
    db.write_file(
        "/project/script.py",
        "# /// script\n# dependencies = ['used-lib']\n# ///\nimport helper\n",
    )?;
    db.write_file("/helpers/helper.py", "import used_lib\n")?;
    let mut paths = SearchPathSettings::new(vec![SystemPathBuf::from("/project")]);
    paths.site_packages_paths = vec![SystemPathBuf::from("/site-packages")];
    paths.extra_paths = vec![SystemPathBuf::from("/helpers")];
    let project = db.project();
    let mut settings = project.program_settings(&db).clone();
    settings.search_paths = paths.to_search_paths(db.system(), db.vendored(), &FallibleStrategy)?;
    project.update_program(&mut db, settings);
...
#[test]
fn incomplete_script_helper_does_not_report_unused_dependencies() -> anyhow::Result<()> {
    let mut db = database(Some("warn"), &["unused_lib"])?;
    db.write_file(
        "/project/script.py",
        "# /// script\n# dependencies = ['unused-lib']\n# ///\nimport helper\n",
    )?;
    db.write_file("/project/helper.py", "def incomplete(\n")?;
    let metadata = metadata(&[(
        "/project/script.py",
        DependencyProjectKind::Script,
        &["unused-lib"],
